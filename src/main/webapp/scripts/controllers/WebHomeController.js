		...
		.controller(
				...
				[
						...
						function($scope, groupData, $location) {
							...
							$scope.columnHeadings = [ "COMMODITY",
									"EXPIRY DATE", "PREVIOUS SELL PRICE",
									"PREVIOUS SELL DATE",
									"PREVIOUS SELL QUANTITY", "ORDER PRICE",
									"ORDER TYPE", "QUANTITY", "DELETE" ];
							...
							$scope.getExpiryDates = function(groupIndex,
									orderIndex, comodityName) {
								var commodity = _
										.find(
												$scope.commodities,
												function(commodity) {
							...
							$scope.changeExpiryDate = function(groupIndex,
									orderIndex) {
								console
										.log($scope.groups[groupIndex].orderData[orderIndex]);
							}

							$scope.deleteCommodity = function(groupIndex,
									orderIndex) {
								$scope.groups[groupIndex].orderData.splice(
										orderIndex, 1);
							}
							...
							$scope.addCommodity = function(groupIndex) {
								...
								$scope.groups[groupIndex].orderData
										.push(newCommodity);
							}
							...
							$scope.postOrder = function() {
								for ( var i = 0; i < $scope.groups.length; i++) {
									for ( var j = 0; j < $scope.groups[i].orderData.length; j++) {
										if (!$scope.groups[i].orderData[j].commodity.name) {
											alert("Please select 'Commodity' in group "
													+ $scope.groups[i].groupName);
											return;
										}
										if (!$scope.groups[i].orderData[j].expiryDate) {
											alert("Please select 'Expirty Date' for "
													+ $scope.groups[i].orderData[j].commodity.name
													+ " in group "
													+ $scope.groups[i].groupName);
											return;
										}
										if (!$scope.groups[i].orderData[j].orderPrice) {
											alert("Please enter 'Oorder Price' for "
													+ $scope.groups[i].orderData[j].commodity.name
													+ " in group "
													+ $scope.groups[i].groupName);
											return;
										...
										if (!$scope.groups[i].orderData[j].quantity) {
											alert("Please enter 'Quantity' for "
													+ $scope.groups[i].orderData[j].commodity.name
													+ " in group "
													+ $scope.groups[i].groupName);
											return;
								...
								sessionStorage.setItem("orderData", JSON
										.stringify($scope.groups));
								sessionStorage.setItem("commodityData", JSON
										.stringify($scope.commodities));
								sessionStorage.setItem("groupData", JSON
										.stringify(groupData));
