...
public class CommoditiesDaoImpl extends BaseDao implements CommodityDao {
	...
	@Override
	public Map<Integer, CommodityVO> getCommodityIntervals(
			Set<Integer> commodityIDsSet) {
		...
		return getJdbcTemplate().query(query,
				new ResultSetExtractor<Map<Integer, CommodityVO>>() {
	...
	@Override
	public List<CommodityFamilyVO> getCommodityFamilies() {
		String query = "select id as id, name as name from commodity_family";
		try {
			return (List<CommodityFamilyVO>) getJdbcTemplate().query(
				query,
				new BeanPropertyRowMapper<CommodityFamilyVO>(
						CommodityFamilyVO.class));
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	@Override
	public List<CommodityVO> getCommoditiesByFamily(Long familyId) {
		String query = "select id, name from commodity where cmdt_family_id=?";
		Object[] params = new Object[] { familyId };
		try {
			return getJdbcTemplate().query(query, params,
				new BeanPropertyRowMapper<CommodityVO>(CommodityVO.class));
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
